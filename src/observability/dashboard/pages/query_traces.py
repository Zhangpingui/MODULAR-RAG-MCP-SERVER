...
def _query_stage_label(stage_name: str) -> str:
    return _QUERY_STAGE_LABELS.get(stage_name, stage_name)
...
def render() -> None:
    ...
    for idx, trace in enumerate(traces):
        ...
        with st.expander(expander_title, expanded=(idx == 0)):
            ...
            if main_timings:
                st.markdown("#### ⏱️ 阶段耗时")
                chart_data = {_query_stage_label(t["stage_name"]): t["elapsed_ms"] for t in main_timings}
                st.bar_chart(chart_data, horizontal=True)
                st.table([
                    {
                        "阶段": _query_stage_label(t["stage_name"]),
                ...
                for tab, (label, key) in zip(tabs, tab_defs):
                    with tab:
                        ...
                        if key == "query_processing":
                            ...
                            _render_retrieval_stage(data, "稠密检索", trace_idx=idx)
                        elif key == "sparse_retrieval":
                            _render_retrieval_stage(data, "稀疏检索", trace_idx=idx)
                        elif key == "fusion":
...
def _render_diagnostics(
    ...
    if dense_err:
        ...
        hints.append(("warning", "稠密检索返回 **0 条结果**，请检查集合是否已建立索引数据。"))
    ...
    if sparse_err:
        ...
        hints.append((
            "warning",
            "稀疏检索（BM25）返回 **0 条结果**。"
            "该集合的 BM25 索引可能为空或尚未构建。",
        ))

    # Fusion missing
    if "fusion" not in stages_by_name:
        if dense_count > 0 and sparse_count > 0:
            hints.append(("info", "稠密与稀疏均有结果，但未记录融合阶段。"))
        elif dense_count == 0 or sparse_count == 0:
            only_source = "稠密" if dense_count > 0 else ("稀疏" if sparse_count > 0 else "都没有")
            hints.append((
                "info",
    ...
    if "rerank" not in stages_by_name:
        if dense_count > 0 or sparse_count > 0:
            hints.append((
                "info",
                "**重排已跳过：** 重排器未启用或未配置。"
                "请在 settings.yaml 中启用重排以使用 LLM 重排。",
            ))
...
def _render_evaluate_button(trace: Dict[str, Any], idx: int) -> None:
    ...
    with col_info:
        if not user_answer.strip():
            ...
            st.caption(
                "将使用 Ragas 评估 faithfulness、answer_relevancy 和 context_precision。"
                "评估会调用 LLM，可能需要几秒钟。"
            )
...
def _render_retrieval_stage(data: Dict[str, Any], label: str, *, trace_idx: int = 0) -> None:
    ...
    with c2:
        ...
        extra_label = "提供方" if "provider" in data else "关键词数"
...
def _render_rerank_stage(data: Dict[str, Any], *, trace_idx: int = 0) -> None:
    ...
    with c1:
        st.metric("方法", data.get("method", "—"))
    with c2:
        st.metric("提供方", data.get("provider", "—"))
    with c3:
